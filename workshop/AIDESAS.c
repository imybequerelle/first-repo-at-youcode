...
#include <stdlib.h>
#include <string.h>
#include <time.h> // Pour la date d'arrivée (Bonus) et la génération d'ID
...
// BONUS: Pour la catégorisation (carnivore/herbivore/omnivore)
typedef enum {
    CARNIVORE,
    HERBIVORE,
    OMNIVORE,
...
typedef struct {
    ...
    char date_arrivee[TAILLE_DATE]; // Bonus: Date d'arrivée
...
void verifier_age_critique(const Animal* a); // Bonus: Age critique
...
/**
 * @brief Convertit une énumération de régime alimentaire en chaîne de caractères.
 */
char* regime_to_string(RegimeAlimentaire regime) {
    switch (regime) {
...
/**
 * @brief Convertit une chaîne de caractères en énumération de régime alimentaire.
 */
RegimeAlimentaire string_to_regime(const char* s) {
...
/**
 * @brief Génère un ID unique et incrémental.
 */
int generer_id() {
    return next_id++;
}
...
/**
 * @brief Trouve l'index d'un animal dans le tableau par son ID.
 * @return L'index de l'animal ou -1 si non trouvé.
 */
int trouver_index_par_id(int id) {
...
// --- Fonctions de Tri (pour qsort) ---

/**
 * @brief Fonction de comparaison pour trier par nom (alphabétique).
 */
int comparer_noms(const void *a, const void *b) {
...
/**
 * @brief Fonction de comparaison pour trier par âge (numérique).
 */
int comparer_ages(const void *a, const void *b) {
...
/**
 * @brief Initialise le tableau du zoo avec des données de test (au moins 20 animaux).
 * REMARQUE: Simule la lecture du fichier animaux.csv.
 */
void initialiser_zoo() {
    // Utilisation d'un tableau statique rempli pour simuler l'initialisation
    // à partir de animaux.csv, comme demandé.
    ajouter_animal_donnees("Leo", "Lion", 15, "Savane", 190.5, CARNIVORE);
    ajouter_animal_donnees("Zara", "Girafe", 7, "Savane", 800.0, HERBIVORE);
    ajouter_animal_donnees("Shere Khan", "Tigre", 9, "Jungle", 220.3, CARNIVORE);
    ajouter_animal_donnees("Manny", "Elephant", 40, "Savane", 5500.0, HERBIVORE);
    ajouter_animal_donnees("Kaa", "Python", 12, "Jungle", 70.2, CARNIVORE);
    ajouter_animal_donnees("Skippy", "Kangourou", 5, "Desert", 65.0, HERBIVORE);
    ajouter_animal_donnees("Flash", "Guepard", 6, "Savane", 55.0, CARNIVORE);
    ajouter_animal_donnees("Bubbles", "Dauphin", 10, "Aquatique", 250.0, CARNIVORE);
    ajouter_animal_donnees("Pongo", "Hippopotame", 25, "Aquatique", 1500.0, HERBIVORE);
    ajouter_animal_donnees("Raja", "Paresseux", 3, "Jungle", 5.5, HERBIVORE);
    ajouter_animal_donnees("Rocky", "Rhinoceros", 18, "Savane", 2000.0, HERBIVORE);
    ajouter_animal_donnees("Shelly", "Tortue", 80, "Aquatique", 450.0, HERBIVORE);
    ajouter_animal_donnees("Coco", "Crocodile", 35, "Aquatique", 600.0, CARNIVORE);
    ajouter_animal_donnees("Miko", "Singe", 8, "Jungle", 15.0, OMNIVORE);
    ajouter_animal_donnees("Baloo", "Ours Brun", 11, "Jungle", 350.0, OMNIVORE);
    ajouter_animal_donnees("Shadow", "Panthere", 10, "Jungle", 60.0, CARNIVORE);
    ajouter_animal_donnees("Iggy", "Iguane", 4, "Desert", 2.5, HERBIVORE);
    ajouter_animal_donnees("Dusty", "Chameau", 14, "Desert", 700.0, HERBIVORE);
    ajouter_animal_donnees("Sting", "Scorpion", 1, "Desert", 0.1, CARNIVORE);
    ajouter_animal_donnees("Patou", "Loup", 7, "Savane", 40.0, CARNIVORE);
    // Un 21ème animal pour assurer le test de la limite et des IDs
    ajouter_animal_donnees("Max", "Panda Roux", 2, "Jungle", 5.0, HERBIVORE);

    printf("✅ Zoo initialisé avec %d animaux de test.\n", nb_animaux);
}

// --- Fonctions de Gestion (CRUD) ---

/**
 * @brief Ajoute un animal au tableau du zoo.
 * @param nom, espece, age, habitat, poids, regime Les données de l'animal.
 */
void ajouter_animal_donnees(const char* nom, const char* espece, int age, const char* habitat, float poids, RegimeAlimentaire regime) {
    if (nb_animaux < MAX_ANIMAUX) {
        ...
        // Bonus: Date d'arrivée automatique
        time_t t = time(NULL);
        ...
        printf("⚠️ Erreur: Limite maximale de %d animaux atteinte.\n", MAX_ANIMAUX);
    }
}

/**
 * @brief Ajoute un ou plusieurs animaux en demandant les informations à l'utilisateur.
 */
void ajouter_animal_interactif() {
    int nb_a_ajouter = 1;
    char choix_multi;
    ...
    for (int i = 0; i < nb_a_ajouter; i++) {
        if (nb_animaux >= MAX_ANIMAUX) {
            printf("⚠️ Limite maximale de %d animaux atteinte. Arrêt de l'ajout.\n", MAX_ANIMAUX);
            break;
        }
        ...
        printf("Âge (entier): ");
        if (scanf("%d", &a.age) != 1) {
            printf("⚠️ Entrée invalide pour l'âge. Annulation de l'ajout.\n");
            while (getchar() != '\n');
        ...
        if (strlen(poids_str) > 0) {
            if (sscanf(poids_str, "%f", &a.poids) != 1) {
                printf("⚠️ Poids invalide. Poids fixé à 0.0.\n");
                a.poids = 0.0;
            }
        } else {
            a.poids = 0.0;
...
/**
 * @brief Affiche la liste des animaux dans un tableau structuré.
 * @param animaux Le tableau d'animaux à afficher.
 * @param taille La taille du tableau.
 */
void afficher_animaux(const Animal animaux[], int taille) {
    if (taille == 0) {
        printf("🤷‍♂️ Aucune animal à afficher.\n");
        return;
    }
...
/**
 * @brief Affiche le sous-menu d'affichage.
 */
void afficher_menu_affichage() {
    int choix;
    do {
...
/**
 * @brief Trie et affiche les animaux par nom.
 */
void trier_animaux_par_nom() {
    // Crée une copie temporaire pour ne pas altérer l'ordre principal du zoo (si ce n'est pas voulu)
...
/**
 * @brief Trie et affiche les animaux par âge.
 */
void trier_animaux_par_age() {
...
/**
 * @brief Affiche les animaux d'un habitat spécifique.
 */
void afficher_animaux_par_habitat() {
...
/**
 * @brief Permet de modifier l'habitat ou l'âge d'un animal.
 */
void modifier_animal_menu() {
    int id;
    int index;
...
/**
 * @brief Supprime un animal par son ID.
 */
void supprimer_animal() {
    int id;
    int index;
...
/**
 * @brief Affiche le menu de recherche.
 */
void rechercher_animal_menu() {
    int choix;
    printf("\n--- Menu Recherche ---\n");
...
/**
 * @brief Affiche les statistiques du zoo.
 */
void afficher_statistiques() {
    printf("\n--- Statistiques du Zoo ---\n");
    ...
    // 5. Espèces les plus représentées (Simple : Top 3 ou toutes)
    // Nécessite une structure pour compter les espèces
    struct {
        char espece[TAILLE_ESPECE];
        int count;
    ...
    // Tri (simple bulle ou qsort) pour afficher les plus représentées en premier
...
/**
 * @brief Vérifie et affiche un message si l'animal atteint un âge critique (ex. > 20 ans pour un lion).
 * @param a Pointeur vers l'animal.
 */
