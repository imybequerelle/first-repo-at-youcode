...
/* Helpers */
void now_date(char buf[DATE_LEN]) {
    time_t t = time(NULL);
    struct tm *tm = localtime(&t);
    snprintf(buf, DATE_LEN, "%04d-%02d-%02d", tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday);
}
...
void now_date(char buf[DATE_LEN]) {
    ...
    // Convertir cette heure en structure (année, mois, jour, etc.)
    struct tm *date_locale = localtime(&maintenant);

    // Remplir la chaîne 'buf' avec la date au format YYYY-MM-DD
    snprintf(buf, DATE_LEN, "%04d-%02d-%02d",
             date_locale->tm_year + 1900,  // année depuis 1900
             date_locale->tm_mon + 1,      // mois (0 = janvier, donc +1)
             date_locale->tm_mday);        // jour du mois
}
...
void statistiques() {
    ...
    if (total > 0) {
        double age_moy = (double)somme_age / total;
        printf("Age moyen: %.2f\n", age_moy);
