...
void ajouter_plusieurs_animaux() {
    int n;
    printf("Combien d'animaux ajouter ? ");
...
void afficher_animal_line(const Animal *a) {
    printf("ID:%3d | %-10s | %-8s | Age:%2d | %-8s | Poids: %6.1f | Arrivee:%s\n",
           a->id, a->nom, a->espece, a->age, a->habitat, (a->poids < 0 ? 0.0 : a->poids), a->date_arrivee);
}
...
void statistiques() {
    ...
    if (total > 0) {
        ...
        // trier freq par cnt decroissant simple selection
